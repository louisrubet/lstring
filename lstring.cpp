...
lstring &lstring::allocateCopy(const lstring &other) {
  if (other.str != nullptr &&
      ((str = (char *)malloc(other.len + 1)) != nullptr)) {
    len = other.len;
    cap = len + 1;
...
lstring &lstring::cat(const lstring &other) {
  if (other.str == nullptr || cap < len + 1)
    return *this;
...
lstring &lstring::cpy(const lstring &other) {
  return cpy(other.str, other.len);
}
...
lstring lstring::toString(bool b, char (&buf)[16]) {
  return b ? lstring("true") : lstring("false");
}
lstring lstring::toString(float f, char (&buf)[16]) {
  return lstring(buf, sizeof(buf)).sprintf("%.02f", f);
}
lstring lstring::toString(int i, char (&buf)[16]) {
  return lstring(buf, sizeof(buf)).sprintf("%d", i);
}
lstring lstring::toString(short i, char (&buf)[16]) {
  return lstring(buf, sizeof(buf)).sprintf("%hd", i);
}
lstring lstring::toString(char i, char (&buf)[16]) {
  return lstring(buf, sizeof(buf)).sprintf("%hhd", i);
}
lstring lstring::toString(unsigned int i, char (&buf)[16]) {
  return lstring(buf, sizeof(buf)).sprintf("%u", i);
}
lstring lstring::toString(unsigned short i, char (&buf)[16]) {
  return lstring(buf, sizeof(buf)).sprintf("%hu", i);
}
lstring lstring::toString(unsigned char i, char (&buf)[16]) {
  return lstring(buf, sizeof(buf)).sprintf("%hhu", i);
}

int lstring::toInt() const {
  int res = 0;
  if (str != nullptr && len > 0)
    (void)sscanf(str, "%d", &res);
  return res;
}
