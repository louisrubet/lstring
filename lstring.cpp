  ...
  public:
    ...
    lstring& cpy(const char* src, size_t src_len) {
        if (src == nullptr || str == nullptr)
            return *this;
        size_t copy_len = (src_len < cap - 1) ? src_len : cap - 1;
        memcpy(this->str, src, copy_len);
        len = copy_len;
        this->str[len] = 0;
        return *this;
    }
    ...
    lstring& trim() {
        ...
        int spaces = 0;
        while (spaces < len && isspace(str[spaces]))
            spaces++;
    ...
    template <size_t N> bool operator==(const char (&other)[N]) {
        if (len != (N - 1))
            return false;
        return strncmp(str, other, N) == 0;
    }

    template <size_t N> bool operator==(const char (&other)[N]) const {
        if (len != (N - 1))
            return false;
        return strncmp(str, other, N) == 0;
    }
